...
class SettingsListViewController: UITableViewController, ManagedObjectContextHolder {
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        if let b = Baby.currentBaby {
            let babyInfoString = NSLocalizedString("settings_baby_info", comment:"Label in the settings pane to display the current baby's info")
            babysAgeLabel.text = NSString(format: babyInfoString,  b.name,  b.birthDate.stringWithHumanizedTimeDifference(false))
        }
    }
